...
//input = input + "Slices" + File.separator; 
  ...
  Dialog.create("Setup");
  Dialog.addMessage("Choose Animal folder within Slices from M2 CropSlide pipeline:");
  Dialog.addDirectory("\n", input);
 Dialog.addMessage("Select the Channel to adjust cells: \nC1 (red), C2 (green), C3 (blue), C4 (farred)");
...
//print(zipPath+File.separator+channelc+"_"+replace(file,".tif",".zip"));
...
run("Point Tool...", "type=Circle color=White size=[Large] label show counter=0");
setTool("multipoint");
roiManager("Select",0);
